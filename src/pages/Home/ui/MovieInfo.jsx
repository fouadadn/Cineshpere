...
export default function MovieInfo({ year, name, Genre, desc }) {
    const y1 = year[0]
    const y2 = year[1]
    let genre1 = ''
    let genre2 = ''
    ...
    return (
        <div className="absolute bottom-16 md:bottom-20  left-10 text-white z-50 ">
            <span className="text-sm hidden sm:block">{y1 == undefined ? String(y2).substring(0, 4) : String(y1).substring(0, 4)}</span>
            <h1 className="text-3xl ">{name}</h1>
            <p className='hidden sm:flex'>{genre1} {xi7aja == '' ? '' : <Dot />} {genre2}</p>
            <p className='w-[370px] sm:w-[450px] md:w-[470px] text-xs sm:text-base'>{desc}</p>
